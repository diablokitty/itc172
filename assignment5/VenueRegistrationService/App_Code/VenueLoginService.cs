...
public class VenueLoginService : IVenueLoginService
{
    ...
    public bool RegisterVenue(VenueMash v)
    {
        bool result = true;
    ...
    public int VenueLogin(string userName, string password)
    {
        ...
        if(result != -1)
        {
            var key = (from vl in db.VenueLogins where vl.VenueLoginUserName.Equals(userName) select new { vl.VenueKey}).FirstOrDefault();
